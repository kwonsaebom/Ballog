...
const TeamSelect = () => {
  ...
  const navigation = useNavigation(); // Initialize navigation
    ...
    const onPressHandler = async () => {
      if (selectedOption) {
        try {
          const accessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJodHRwczovL2FwaS5iYWxsb2cuc3RvcmUiLCJzdWIiOiJ0ZXN0MiIsImlhdCI6MTcyNDkzNzE2NiwiZXhwIjoxNzMwMTIxMTY2fQ.r29UK6qbIj8__WSif36FP1u6zpIrcMyAeVH-DhiIgVo'; // 실제 토큰으로 교체
  
          const response = await axios.patch('https://api.ballog.store/myPage/setting/teamSetting', 
            {
          ...
          {teamData.length === 10 ? (
            teamData.map((team, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionButton, 
                  selectedOption?.team_id === team.team_id && styles.selectedOptionButton
                ]}
                onPress={() => setSelectedOption(team)} // 팀 선택
              >
                <Text style={styles.optionButtonText}>{team.team_name}</Text>
                <Image source={{ uri: team.team_icon_flag }} style={styles.optionImage} /> 
              </TouchableOpacity>
             ))
            ) : (
